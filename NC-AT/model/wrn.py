...
class WideResNet(nn.Module):
    def __init__(self, depth, widen_factor, num_classes=200):
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(out, 8 if out.size(-1) == 32 else 16)
...
#WideResNet-28-10
def wide_resnet_28_10_imagenet(num_classes):
    model = WideResNet(depth=28, widen_factor=10 ,num_classes=num_classes)
    return model

def wide_resnet_28_10_cifar(num_classes):
    model = Wide_ResNet(depth=28, num_classes=num_classes, widen_factor=10, dropRate=0.0)
    return model


def return_wrn(num_classes, dataset):
    if dataset == 'imagenet':
        return wide_resnet_28_10_imagenet(num_classes)
    else:
        return wide_resnet_28_10_cifar(num_classes)
